...
class Client(metaclass=ClientVerifier):
    ...
    def parse_response(self, message):
        ...
        if os.getenv('RESPONSE') in message:
            if message[os.getenv('RESPONSE')] == 200:
                print("/\/CLN_SM_1", "['RESPONSE':]", os.getenv('RESPONSE'))
                return 200
    ...
    def connect_to_server(self, sock):
        try:
            ...
            self.CLIENT_LOGGER.critical(
                f'Не удалось подключиться к серверу {self.serv_addr}:{self.dest_port}, '
    ...
    def get_addr_port(self):
        ...
        self.dest_port = args.p
        self.client_name = args.n
        if not 1023 < self.dest_port < 65536:
            self.CLIENT_LOGGER.critical(f"Попытка запуска клиента с неподходящим номером порта удалённого сервера: {self.dest_port}. "
            ...
            # print(f"Попытка запуска клиента с неподходящим номером порта удалённого  сервера: {self.dest_port}. "
            #     f"Допустимы адреса с 1024 до 65535. Клиенту будет принудительно установлен порт удалённого серверапо умолчанию : {os.getenv('DEFAULT_PORT')}")
            self.dest_port = os.getenv('DEFAULT_PORT')
    ...
    def start_client(self):
        ...
        # global serv_addr, dest_port

        # Получаем параметры подключения
        self.get_addr_port()


        self.CLIENT_LOGGER.info(f'Запущен клиент с параметрами: адрес сервера: {self.serv_addr}, '
            f'порт: {self.dest_port}')
