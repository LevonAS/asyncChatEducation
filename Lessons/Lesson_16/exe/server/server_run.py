...
class Server(threading.Thread):
    ...
    def process_client_message(self, message,  client):
        ...
        # Если это сообщение, то отправляем его получателю.
            ...
            if message[os.getenv('DESTINATION')] in self.names:
                self.database.process_message(
                    message[os.getenv('SENDER')], message[os.getenv('DESTINATION')])
        ...
        # Если это запрос контакт-листа
            ...
            response = {os.getenv('RESPONSE'): 202,
                        os.getenv('LIST_INFO'): None}
            print("/\/SERV_PCMGC_1 [response:]", response)
            response[os.getenv('LIST_INFO')] = self.database.get_contacts(
                message[os.getenv('USER')])
            print("/\/SERV_PCMGC_2 [response:]", response)
            try:
                self.send_message(client, response)
                print("/\/SERV_PCMGC_3 [response:]",
                      response, "\n[client:]", client)
            except OSError:
                self.remove_client(client)
        ...
        # Если это добавление контакта
            ...
            self.database.add_contact(
                message[os.getenv('USER')], message[os.getenv('ACCOUNT_NAME')])
            try:
                self.send_message(client, {os.getenv('RESPONSE'): 200})
            except OSError:
                self.remove_client(client)
        ...
        # Если это запрос публичного ключа пользователя
            ...
            response[os.getenv('DATA')] = self.database.get_pubkey(
    ...
    def autorize_user(self, message, sock):
        ...
        self.SERVER_LOGGER.debug(
            f"Start auth process for {message[os.getenv('USER')]}")
        ...
        # Проверяем что пользователь зарегистрирован на сервере.
            ...
            try:
                self.SERVER_LOGGER.debug(
                    f'Unknown username, sending {response}')
                self.send_message(sock, response)
            except OSError:
                pass
            self.clients.remove(sock)
            ...
            self.SERVER_LOGGER.debug(
                'Correct username, starting passwd check.')
            ...
            hash = hmac.new(self.database.get_hash(
                message[os.getenv('USER')][os.getenv('ACCOUNT_NAME')]), random_str, 'MD5')
            digest = hash.digest()
            ...
            if os.getenv('RESPONSE') in ans and ans[os.getenv('RESPONSE')] == 511 and hmac.compare_digest(
                ...
                self.names[message[os.getenv(
                    'USER')][os.getenv('ACCOUNT_NAME')]] = sock
    ...
    def service_update_lists(self):
        ...
        for client in self.names:
            try:
                self.send_message(self.names[client], {
                                  os.getenv('RESPONSE'): 205})
            except OSError:
    ...
    def send_data(self, message):
        ...
        if message[os.getenv('DESTINATION')] in self.names \
            ...
            try:
                self.send_message(
                    self.names[message[os.getenv('DESTINATION')]], message)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                self.SERVER_LOGGER.info(
                    f'Получен запрос на соединение от {client_address}')
            ...
            try:
                if self.clients:
                    rlist, self.wlist, err_lst = select.select(
...
def srv_args_parser(default_address, default_port):
    ...
    res_addres = default_address if default_address else os.getenv(
        'DEFAULT_IP_ADDRESS')
...
def config_load():
    ...
    dir_path = os.getcwd()
    print("///SERV_mainCF_0: ", " dir_path: ", ',', dir_path)
